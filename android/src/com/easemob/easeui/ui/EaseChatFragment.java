...
public class EaseChatFragment extends EaseBaseFragment implements EMEventListener {
    ...
    protected int[] itemStrings = { RHelper.get("string.attach_take_pic"), RHelper.get("string.attach_picture"), RHelper.get("string.attach_location") };
    protected int[] itemdrawables = { RHelper.get("drawable.ease_chat_takepic_selector"), RHelper.get("drawable.ease_chat_image_selector"),
            RHelper.get("drawable.ease_chat_location_selector") };
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
    	try{
        	return inflater.inflate(TiRHelper.getApplicationResource("layout.ease_fragment_chat"), container, false);
        }
        catch(Exception e){
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        Log.d("fuck1", toChatUsername);
        
        Log.d("fuck2", fragmentArgs.getString("userId"));
    ...
    protected void setUpView() {
        ...
        if (chatType == EaseConstant.CHATTYPE_SINGLE) { // 单聊
            ...
            titleBar.setRightImageResource(RHelper.get("drawable.ease_mm_title_remove"));
            ...
            if (chatType == EaseConstant.CHATTYPE_GROUP) {
                ...
                titleBar.setRightImageResource(RHelper.get("drawable.ease_to_group_details_normal"));
                // 监听当前会话的群聊解散被T事件
    ...
    protected void onMessageListInit(){
    	Log.d("fuck2222", toChatUsername);
    	Log.d("fuck2222", String.valueOf(chatType));
    ...
    protected void setListItemClickListener() {
        messageList.setItemClickListener(new EaseChatMessageList.MessageListItemClickListener() {
            ...
            public void onResendClick(final EMMessage message) {
                new EaseAlertDialog(getActivity(), RHelper.get("string.resend"), RHelper.get("string.confirm_resend"), null, new AlertDialogUser() {
                    @Override
                ...
                new Handler().postDelayed(new Runnable() {
                    ...
                    public void run() {
                        if (listView.getFirstVisiblePosition() == 0 && !isloading && haveMoreData) {
                            ...
                            Toast.makeText(getActivity(), getResources().getString(RHelper.get("string.no_more_messages")),
        ...
        if (resultCode == Activity.RESULT_OK) { 
            if (requestCode == REQUEST_CODE_CAMERA) { // 发送照片
                ...
                if (locationAddress != null && !locationAddress.equals("")) {
                    ...
                    Toast.makeText(getActivity(), RHelper.get("string.unable_to_get_loaction"), 0).show();
                }
    ...
    protected void sendPicByUri(Uri selectedImage) {
        ...
        if (cursor != null) {
            ...
            if (picturePath == null || picturePath.equals("null")) {
                Toast toast = Toast.makeText(getActivity(), RHelper.get("string.cant_find_pictures"), Toast.LENGTH_SHORT);
    ...
    protected void sendFileByUri(Uri uri){
        ...
        if (file == null || !file.exists()) {
            Toast.makeText(getActivity(), RHelper.get("string.File_does_not_exist"), 0).show();
            return;
        }
        //大于10M不让发送
        if (file.length() > 10 * 1024 * 1024) {
            Toast.makeText(getActivity(), RHelper.get("string.The_file_is_not_greater_than_10_m"), 0).show();
            return;
        }
    ...
    protected void selectPicFromCamera() {
        if (!EaseCommonUtils.isExitsSdcard()) {
            Toast.makeText(getActivity(), RHelper.get("string.sd_card_does_not_exist"), 0).show();
            return;
        }
    ...
    protected void emptyHistory() {
        String msg = getResources().getString(RHelper.get("string.Whether_to_empty_all_chats"));
    ...
    protected void toGroupDetails() {
        if (chatType == EaseConstant.CHATTYPE_GROUP) {
            EMGroup group = EMGroupManager.getInstance().getGroup(toChatUsername);
            if (group == null) {
                Toast.makeText(getActivity(), RHelper.get("string.gorup_not_found"), 0).show();
                return;
            }
            if(chatFragmentListener != null){
    ...
    class GroupListener extends EaseGroupRemoveListener {
            ...
            getActivity().runOnUiThread(new Runnable() {
                ...
                public void run() {
                    if (toChatUsername.equals(groupId)) {
                        Toast.makeText(getActivity(), RHelper.get("string.you_are_group"), 1).show();
                        getActivity().finish();
                    }
                }
            });
        ...
        public void onGroupDestroy(final String groupId, String groupName) {
            ...
            getActivity().runOnUiThread(new Runnable() {
                public void run() {
                    if (toChatUsername.equals(groupId)) {
                        Toast.makeText(getActivity(), RHelper.get("string.the_current_group"), 1).show();
                        getActivity().finish();
                    }
                }
            });
