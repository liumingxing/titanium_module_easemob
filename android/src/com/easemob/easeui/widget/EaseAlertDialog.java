...
public class EaseAlertDialog extends Dialog {
	...
	public EaseAlertDialog(Context context, int msgId) {
		super(context);
		this.title = context.getResources().getString(RHelper.get("string.prompt"));
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(RHelper.get("layout.ease_alert_dialog"));
		
		Button cancel;
		Button ok;
		TextView titleView;
		try{
			cancel = (Button)findViewById(TiRHelper.getApplicationResource("id.btn_cancel"));
			ok = (Button)findViewById(TiRHelper.getApplicationResource("id.btn_ok"));
			...
			View.OnClickListener listener = new View.OnClickListener() {
				...
				public void onClick(View view) {
					if (view.getId() == RHelper.get("id.btn_ok")) {
						onOk(view);
					} else if (view.getId() == RHelper.get("id.btn_cancel")) {
						onCancel(view);
					}
				}
			};
			...
			if (msg != null)
			    ((TextView)findViewById(TiRHelper.getApplicationResource("id.alert_message"))).setText(msg);
		}
		catch(Exception e){
		}
	}
