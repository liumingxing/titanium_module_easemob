...
public abstract class EaseChatRow extends LinearLayout {
    ...
    private void initView() {
        onInflatView();
        try{
        timeStampView = (TextView) findViewById(TiRHelper.getApplicationResource("id.timestamp"));
        userAvatarView = (ImageView) findViewById(TiRHelper.getApplicationResource("id.iv_userhead"));
        bubbleLayout = findViewById(TiRHelper.getApplicationResource("id.bubble"));
        usernickView = (TextView) findViewById(TiRHelper.getApplicationResource("id.tv_userid"));

        progressBar = (ProgressBar) findViewById(TiRHelper.getApplicationResource("id.progress_bar"));
        statusView = (ImageView) findViewById(TiRHelper.getApplicationResource("id.msg_status"));
        ackedView = (TextView) findViewById(TiRHelper.getApplicationResource("id.tv_ack"));
        deliveredView = (TextView) findViewById(TiRHelper.getApplicationResource("id.tv_delivered"));
		}
		catch(Exception e){
		}
        onFindViewById();
    ...
    private void setUpBaseView() {
        // 设置用户昵称头像，bubble背景等
        try{
	        TextView timestamp = (TextView) findViewById(TiRHelper.getApplicationResource("id.timestamp"));
	        if (timestamp != null) {
	            if (position == 0) {
            ...
            public void run() {
                if (message.status == EMMessage.Status.FAIL) {
                    ...
                    if (message.getError() == EMError.MESSAGE_SEND_INVALID_CONTENT) {
                        Toast.makeText(activity,activity.getString(RHelper.get("string.send_fail")) + activity.getString(RHelper.get("string.error_send_invalid_content")), 0).show();
                    } else if (message.getError() == EMError.MESSAGE_SEND_NOT_IN_THE_GROUP) {
                        Toast.makeText(activity,activity.getString(RHelper.get("string.send_fail")) + activity.getString(RHelper.get("string.error_send_not_in_the_group")), 0).show();
                    } else {
                        Toast.makeText(activity,activity.getString(RHelper.get("string.send_fail")) + activity.getString(RHelper.get("string.connect_failuer_toast")), 0).show();
                    }
                }
