...
//              digest = EasyUtils.getAppResourceString(context, "location_recv");
                digest = getString(context, RHelper.get("string.location_recv"));
                digest = String.format(digest, message.getFrom());
                return digest;
            } else {
//              digest = EasyUtils.getAppResourceString(context, "location_prefix");
                digest = getString(context, RHelper.get("string.location_prefix"));
            }
            break;
        case IMAGE: // 图片消息
            digest = getString(context, RHelper.get("string.picture"));
            break;
        case VOICE:// 语音消息
            digest = getString(context, RHelper.get("string.voice_prefix"));
            break;
        case VIDEO: // 视频消息
            digest = getString(context, RHelper.get("string.video"));
            break;
        case TXT: // 文本消息
            /*if(((DemoHXSDKHelper)HXSDKHelper.getInstance()).isRobotMenuMessage(message)){
                ...
                TextMessageBody txtBody = (TextMessageBody) message.getBody();
                digest = getString(context, RHelper.get("string.voice_call")) + txtBody.getMessage();
            }
            break;
        case FILE: //普通文件消息
            digest = getString(context, RHelper.get("string.file"));
            break;
        default:
