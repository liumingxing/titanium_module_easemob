...
public class GroupBlacklistActivity extends BaseActivity {
	...
	protected void onCreate(Bundle arg0) {
		super.onCreate(arg0);
		setContentView(RHelper.get("layout.em_activity_group_blacklist"));

		progressBar = (ProgressBar) findViewById(RHelper.get("id.progressBar"));
		listView = (ListView) findViewById(RHelper.get("id.list"));

		groupId = getIntent().getStringExtra("groupId");
		// 注册上下文菜单
		registerForContextMenu(listView);
		final String st1 = getResources().getString(RHelper.get("string.get_failed_please_check"));
	...
	void removeOutBlacklist(final String tobeRemoveUser) {
		final String st2 = getResources().getString(RHelper.get("string.Removed_from_the_failure"));
		try {
			// 移出黑民单
