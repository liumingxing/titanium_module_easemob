...
public class ChatRoomDetailsActivity extends BaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		blockGroupMsgLayout = (RelativeLayout)findViewById(RHelper.get("id.rl_switch_block_groupmsg"));
		showChatRoomIdLayout = (RelativeLayout)findViewById(RHelper.get("id.rl_group_id"));
		chatRoomIdTextView = (TextView)findViewById(RHelper.get("id.tv_group_id"));

		Drawable referenceDrawable = getResources().getDrawable(RHelper.get("drawable.em_smiley_add_btn"));
		...
		((TextView) findViewById(RHelper.get("id.group_name"))).setText(room.getName() + "(" + room.getAffiliationsCount() + st);
		adapter = new GridAdapter(this, RHelper.get("layout.em_grid"), room.getMembers());
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		String st8 = getResources().getString(RHelper.get("string.Are_moving_to_blacklist"));
		final String st9 = getResources().getString(RHelper.get("string.failed_to_move_into"));
		
		final String stsuccess = getResources().getString(RHelper.get("string.Move_into_blacklist_success"));
		...
		new Thread(new Runnable() {
			public void run() {
				try {
					...
					runOnUiThread(new Runnable() {
						public void run() {
							((TextView) findViewById(RHelper.get("id.group_name"))).setText(returnRoom.getName() + "(" + returnRoom.getAffiliationsCount()
									+ "人)");
	...
	public void onClick(View v) {
		if (v.getId() == RHelper.get("id.clear_all_history")) {
			// 清空聊天记录
					...
					public void onClick(View v) {
						if (isInDeleteMode) {
							...
							if (EMChatManager.getInstance().getCurrentUser().equals(username)) {
							    new EaseAlertDialog(ChatRoomDetailsActivity.this, RHelper.get("string.not_delete_myself")).show();
								return;
							}
