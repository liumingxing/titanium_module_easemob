...
public class GroupDetailsActivity extends BaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		rl_switch_block_groupmsg = (RelativeLayout) findViewById(RHelper.get("id.rl_switch_block_groupmsg"));
		...
		((TextView) findViewById(RHelper.get("id.group_name"))).setText(group.getGroupName() + "(" + group.getAffiliationsCount() + st);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		String st1 = getResources().getString(RHelper.get("string.being_added"));
		String st2 = getResources().getString(RHelper.get("string.is_quit_the_group_chat"));
		String st3 = getResources().getString(RHelper.get("string.chatting_is_dissolution"));
		String st4 = getResources().getString(RHelper.get("string.are_empty_group_of_news"));
		String st5 = getResources().getString(RHelper.get("string.is_modify_the_group_name"));
		final String st6 = getResources().getString(RHelper.get("string.Modify_the_group_name_successful"));
		final String st7 = getResources().getString(RHelper.get("string.change_the_group_name_failed_please"));
						...
						public void run() {
							try {
								...
								runOnUiThread(new Runnable() {
									public void run() {
										((TextView) findViewById(RHelper.get("id.group_name"))).setText(returnData + "(" + group.getAffiliationsCount()
												+ st);
    ...
    protected void addUserToBlackList(final String username) {
        ...
        pd.setMessage(getString(RHelper.get("string.Are_moving_to_blacklist")));
        pd.show();
        new Thread(new Runnable() {
        	public void run() {
        		try {
        			...
        			runOnUiThread(new Runnable() {
        				public void run() {
        				    refreshMembers();
        				    pd.dismiss();
        					Toast.makeText(getApplicationContext(), RHelper.get("string.Move_into_blacklist_success"), 0).show();
        				}
        			});
        		} catch (EaseMobException e) {
        			runOnUiThread(new Runnable() {
        				public void run() {
        				    pd.dismiss();
        					Toast.makeText(getApplicationContext(), RHelper.get("string.failed_to_move_into"), 0).show();
        				}
        			});
        		}
        	}
	...
	public void exitDeleteGroup(View view) {
		startActivityForResult(new Intent(this, ExitGroupDialog.class).putExtra("deleteToast", getString(RHelper.get("string.dissolution_group_hint"))),
	...
	private void exitGrop() {
		String st1 = getResources().getString(RHelper.get("string.Exit_the_group_chat_failure"));
		new Thread(new Runnable() {
			public void run() {
				try {
					...
					runOnUiThread(new Runnable() {
						public void run() {
							progressDialog.dismiss();
							Toast.makeText(getApplicationContext(), getResources().getString(RHelper.get("string.Exit_the_group_chat_failure")) + " " + e.getMessage(), 1).show();
						}
					});
				}
			}
	...
	private void deleteGrop() {
		final String st5 = getResources().getString(RHelper.get("string.Dissolve_group_chat_tofail"));
		new Thread(new Runnable() {
			public void run() {
				try {
	...
	private void addMembersToGroup(final String[] newmembers) {
		final String st6 = getResources().getString(RHelper.get("string.Add_group_members_fail"));
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					runOnUiThread(new Runnable() {
						public void run() {
						    refreshMembers();
							((TextView) findViewById(RHelper.get("id.group_name"))).setText(group.getGroupName() + "(" + group.getAffiliationsCount()
									+ st);
							progressDialog.dismiss();
						}
					});
		        ...
		        public void run() {
		            try {
		                ...
		                runOnUiThread(new Runnable() {
		                    public void run() {
		                        progressDialog.dismiss();
		                        Toast.makeText(getApplicationContext(), RHelper.get("string.remove_group_of"), 1).show();
		                    }
		                });
	...
	private class GridAdapter extends ArrayAdapter<String> {
		...
		public View getView(final int position, View convertView, final ViewGroup parent) {
			...
			if (convertView == null) {
				...
				holder.badgeDeleteView = (ImageView) convertView.findViewById(RHelper.get("id.badge_delete"));
				convertView.setTag(holder);
			}else{
			    holder = (ViewHolder) convertView.getTag();
			}
			final LinearLayout button = (LinearLayout) convertView.findViewById(RHelper.get("id.button_avatar"));
			// 最后一个item，减人按钮
			if (position == getCount() - 1) {
			    holder.textView.setText("");
				// 设置成删除按钮
			    holder.imageView.setImageResource(RHelper.get("drawabledrawable.em_smiley_minus_btn"));
//				button.setCompoundDrawablesWithIntrinsicBounds(0, RHelper.get("drawabledrawable.smiley_minus_btn"), 0, 0);
				...
				if (!group.getOwner().equals(EMChatManager.getInstance().getCurrentUser())) {
					...
					final String st10 = getResources().getString(RHelper.get("string.The_delete_button_is_clicked"));
...
//				button.setCompoundDrawablesWithIntrinsicBounds(0, RHelper.get("drawable.smiley_add_btn"), 0, 0);
				...
				if (!group.isAllowInvites() && !group.getOwner().equals(EMChatManager.getInstance().getCurrentUser())) {
					...
					final String st11 = getResources().getString(RHelper.get("string.Add_a_button_was_clicked"));
					button.setOnClickListener(new OnClickListener() {
						@Override
						public void onClick(View v) {
							EMLog.d(TAG, st11);
					...
					public void onClick(View v) {
						if (isInDeleteMode) {
							...
							if (!NetUtils.hasNetwork(getApplicationContext())) {
								Toast.makeText(getApplicationContext(), getString(RHelper.get("string.network_unavailable")), 0).show();
								return;
							}
								...
								try {
									...
									runOnUiThread(new Runnable() {
										...
										public void run() {
											...
											refreshMembers();
											((TextView) findViewById(RHelper.get("id.group_name"))).setText(group.getGroupName() + "("
