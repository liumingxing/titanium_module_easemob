        ...
        if (imageBody.thumbnailDownloadStatus == EMAttachmentDownloadSuccessed) {
            if (imageBody.attachmentDownloadStatus == EMAttachmentDownloadSuccessed)
            {
                ...
                if (localPath && localPath.length > 0) {
                    ...
                    if (image)
                    {
                        [self.messageReadManager showBrowserWithImages:@[image] viewController:self];
                    }
                    else
                    {
            ...
            [weakSelf showHudInView:weakSelf.view hint:@"下载图片"];
            [chatManager asyncFetchMessage:model.message progress:nil completion:^(EMMessage *aMessage, EMError *error) {
                [weakSelf hideHud];
                if (!error) {
                    //发送已读回执
                    ...
                    if (localPath && localPath.length > 0) {
                        ...
                        if (image)
                        {
                            [weakSelf.messageReadManager showBrowserWithImages:@[image] viewController:self];
                        }
                        else
                        {
                            NSLog(@"Read %@ failed!", localPath);
...
- (void)didFinishRecoingVoiceAction:(UIView *)recordView
{
    ...
    [[EMCDDeviceManager sharedInstance] asyncStopRecordingWithCompletion:^(NSString *recordPath, NSInteger aDuration, NSError *error) {
        if (!error) {
            ...
            [weakSelf showHudInView:self.view hint:@"media.timeShort"];
